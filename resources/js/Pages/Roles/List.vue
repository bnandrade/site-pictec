    ...
    <jet-action-section class="sm:mt-0">
        ...
        <template #content>
            <div class="flex justify-between mb-4">
                <h1 class="my-2 font-semibold">Lista de Grupos de Permissões</h1>
            ...
            <div class="space-y-6" v-if="roles.data.length > 0">
                <div class="flex" >
                    <div class="w-full flex flex-col items-start ">
                        <table class="table-fixed text-left w-full">
                            ...
                            <tbody>
                            <tr v-for="(role, index) in roles.data" class="hover:bg-gray-100" :key="role.id" >
                                <td class=" border border-light-blue-500 px-4 py-2 text-light-blue-600 font-medium">{{ role.name }}</td>
                                <td class=" border border-light-blue-500 px-4 py-2 text-light-blue-600 font-medium text-center"><item :permissions="permissions"  :permissions_checked="role.permissions_checked" :role="role"></item></td>
                            </tr>
                            </tbody>
                        </table>
    ...
    watch: {
        form: {
            handler: _.debounce(function() {
                ...
                let route = this.route('roles', Object.keys(query).length ? query : { remember: 'forget' });
